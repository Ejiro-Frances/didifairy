...
export default function CheckoutForm({ initial }: { initial?: CheckoutInitial }) {
        ...
        <section className="flex-1 rounded-2xl border border-[#E8D5A3] bg-white p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="First name" name="firstName" value={form.firstName} onChange={set('firstName')} required error={errors.firstName} />
              <Field label="Last name" name="lastName" value={form.lastName} onChange={set('lastName')} required error={errors.lastName} />
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Email" name="email" type="email" value={form.email} onChange={set('email')} required error={errors.email} />
              <Field label="Phone" name="phone" type="tel" value={form.phone} onChange={set('phone')} required error={errors.phone} />
            </div>
            <TextareaField label="Delivery address" name="address" value={form.address} onChange={set('address')} required error={errors.address} />
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="City" name="city" value={form.city} onChange={set('city')} required error={errors.city} />
              <Field label="State" name="state" value={form.state} onChange={set('state')} required error={errors.state} />
            </div>
